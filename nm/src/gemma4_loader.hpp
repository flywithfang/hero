...
GemmaQueryOutput<Cfg::D, Cfg::Hq, HeadDim> load_query_output(const GGUF& gguf, size_t layer) {
    ...
    return GemmaQueryOutput<Cfg::D, Cfg::Hq, HeadDim>(
        Linear<Cfg::D, QW>(tensor_loader::load_weight<Cfg::D, QW>(gguf, block(layer, "attn_q.weight"))),
        PerHeadNorm<Cfg::Hq, HeadDim, RMSNorm<HeadDim>>(
            RMSNorm<HeadDim>(tensor_loader::load_vector<HeadDim>(gguf, block(layer, "attn_q_norm.weight")), Cfg::RMS_EPS)),
        Linear<QW, Cfg::D>(tensor_loader::load_weight<QW, Cfg::D>(gguf, block(layer, "attn_output.weight"))));
}
...
GemmaKeyValue<Cfg::D, Hkv, HeadDim> load_key_value(const GGUF& gguf, size_t layer) {
    ...
    return GemmaKeyValue<Cfg::D, Hkv, HeadDim>(
        Linear<Cfg::D, KW>(tensor_loader::load_weight<Cfg::D, KW>(gguf, block(layer, "attn_k.weight"))),
        PerHeadNorm<Hkv, HeadDim, RMSNorm<HeadDim>>(
            RMSNorm<HeadDim>(tensor_loader::load_vector<HeadDim>(gguf, block(layer, "attn_k_norm.weight")), Cfg::RMS_EPS)),
        Linear<Cfg::D, KW>(tensor_loader::load_weight<Cfg::D, KW>(gguf, block(layer, "attn_v.weight"))),
        PerHeadNorm<Hkv, HeadDim, RMSNormNoScale<HeadDim>>(
            RMSNormNoScale<HeadDim>(Cfg::RMS_EPS)));
}
...
GemmaUnifiedKeyValue<Cfg::D, Hkv, HeadDim> load_unified_key_value(const GGUF& gguf, size_t layer) {
    constexpr size_t KW = Hkv * HeadDim;
    expect(gguf.find(block(layer, "attn_v.weight")) == nullptr,
           block(layer, "attn_v.weight") +
           " is present, but this layer is configured for unified K/V");
    return GemmaUnifiedKeyValue<Cfg::D, Hkv, HeadDim>(
        Linear<Cfg::D, KW>(tensor_loader::load_weight<Cfg::D, KW>(gguf, block(layer, "attn_k.weight"))),
        PerHeadNorm<Hkv, HeadDim, RMSNorm<HeadDim>>(
            RMSNorm<HeadDim>(tensor_loader::load_vector<HeadDim>(gguf, block(layer, "attn_k_norm.weight")), Cfg::RMS_EPS)),
        PerHeadNorm<Hkv, HeadDim, RMSNormNoScale<HeadDim>>(
...
GemmaE4BDenseLayer<Attention> load_dense_layer(const GGUF& gguf, size_t layer,
    ...
    return GemmaE4BDenseLayer<Attention>(
        ...
        std::move(attention),
        norm("post_attention_norm.weight"),
        norm("ffn_norm.weight"),
        std::move(mlp),
        norm("post_ffw_norm.weight"),
        GemmaPerLayerTail<C::D, C::PLE>(
            std::move(ple),
...
inline GemmaE4BLayer load_layer(const GGUF& gguf, size_t layer) {
    ...
    if (!full && !shared) {
        GemmaE4BLocalOwnAttention attention(
            load_query_output<C, C::LOCAL_HEAD_DIM>(gguf, layer),
            load_key_value<C, C::Hkv, C::LOCAL_HEAD_DIM>(gguf, layer));
        return load_dense_layer(gguf, layer, std::move(attention));
    }
    if (full && !shared) {
        GemmaE4BGlobalOwnAttention attention(
            load_query_output<C, C::GLOBAL_HEAD_DIM>(gguf, layer),
            load_key_value<C, C::Hkv, C::GLOBAL_HEAD_DIM>(gguf, layer));
        return load_dense_layer(gguf, layer, std::move(attention));
    }
    if (!full) {
...
inline Gemma4E4BTransformer load_e4b_text(const GGUF& gguf) {
    ...
    Gemma4E4BTextWeights weights(
        std::move(token_io), std::move(layers),
...
inline void expect_per_layer(const GGUF& gguf, const std::string& key,
                             size_t layers, size_t (*expected)(size_t)) {
    expect(gguf.has(key), "missing metadata key " + key);
    const GGUFValue& value = gguf.get(key);
    if (value.type == GGUFType::ARRAY) {
        expect(value.arr_num.size() >= layers,
               key + " array is shorter than block_count");
        for (size_t layer = 0; layer < layers; ++layer)
            expect(size_t(value.arr_num[layer]) == expected(layer),
                   key + "[" + std::to_string(layer) + "]=" +
                   std::to_string(size_t(value.arr_num[layer])) + ", expected " +
                   std::to_string(expected(layer)));
        return;
    }
    for (size_t layer = 0; layer < layers; ++layer)
        expect(size_t(value.as_int()) == expected(layer),
               key + "=" + std::to_string(value.as_int()) + " but layer " +
               std::to_string(layer) + " expects " + std::to_string(expected(layer)));
}
...
inline void validate_12b_text_metadata(const GGUF& gguf) {
    ...
    expect_per_layer(gguf, "gemma4.attention.head_count_kv", C12::L,
                     [](size_t layer) { return C12::kv_heads(layer); });
    ...
    if (gguf.has("gemma4.attention.shared_kv_layers"))
        expect_integer(gguf, "gemma4.attention.shared_kv_layers", 0);
    if (gguf.has("gemma4.embedding_length_per_layer_input"))
        expect_integer(gguf, "gemma4.embedding_length_per_layer_input", 0);
    expect(gguf.find("per_layer_token_embd.weight") == nullptr,
           "checkpoint carries PLE tensors, which Gemma 4 12B does not have");

    // The sliding/global schedule is metadata; ours is compiled in.
    if (gguf.has("gemma4.attention.sliding_window_pattern"))
        expect_per_layer(gguf, "gemma4.attention.sliding_window_pattern", C12::L,
                         [](size_t layer) -> size_t {
                             return C12::attention_kind(layer) ==
                                    GemmaAttentionKind::Sliding ? 1 : 0;
                         });
    if (gguf.has("tokenizer.ggml.tokens"))
        expect(gguf.get("tokenizer.ggml.tokens").arr_str.size() == C12::V,
               "tokenizer vocabulary size differs from 262144");
}

template <class Attention>
Gemma12BDenseLayer<Attention> load_12b_dense_layer(const GGUF& gguf, size_t layer,
                                                   Attention attention) {
    auto norm = [&](const char* suffix) {
        return RMSNorm<C12::D>(
            tensor_loader::load_vector<C12::D>(gguf, block(layer, suffix)), C12::RMS_EPS);
    };
    GeluGatedMLP<C12::D, C12::FF> mlp(
        Linear<C12::D, C12::FF>(tensor_loader::load_weight<C12::D, C12::FF>(gguf, block(layer, "ffn_gate.weight"))),
        Linear<C12::D, C12::FF>(tensor_loader::load_weight<C12::D, C12::FF>(gguf, block(layer, "ffn_up.weight"))),
        Linear<C12::FF, C12::D>(tensor_loader::load_weight<C12::FF, C12::D>(gguf, block(layer, "ffn_down.weight"))));
    return Gemma12BDenseLayer<Attention>(
        norm("attn_norm.weight"), std::move(attention),
        norm("post_attention_norm.weight"), norm("ffn_norm.weight"),
        std::move(mlp), norm("post_ffw_norm.weight"),
        // No PLE, but the per-layer output scalar is still present.
        GemmaLayerScaleTail<C12::D>(
            optional_scalar(gguf, block(layer, "layer_output_scale.weight"), 1.f)));
}

inline Gemma12BLayer load_12b_layer(const GGUF& gguf, size_t layer) {
    if (C12::attention_kind(layer) == GemmaAttentionKind::Sliding)
        return load_12b_dense_layer(
            gguf, layer,
            Gemma12BSlidingAttention(
                load_query_output<C12, C12::LOCAL_HEAD_DIM>(gguf, layer),
                load_key_value<C12, C12::LOCAL_HKV, C12::LOCAL_HEAD_DIM>(gguf, layer)));
    return load_12b_dense_layer(
        gguf, layer,
        Gemma12BGlobalAttention(
            load_query_output<C12, C12::GLOBAL_HEAD_DIM>(gguf, layer),
            load_unified_key_value<C12, C12::GLOBAL_HKV, C12::GLOBAL_HEAD_DIM>(gguf, layer)));
}

inline Gemma4_12BTransformer load_12b_text(const GGUF& gguf) {
    validate_12b_text_metadata(gguf);

    GemmaTokenIO<C12::D, C12::V> token_io(
        tensor_loader::load_weight<C12::D, C12::V>(gguf, "token_embd.weight"),
        std::sqrt(Scalar(C12::D)));

    std::vector<Gemma12BLayer> layers;
    layers.reserve(C12::L);
    for (size_t layer = 0; layer < C12::L; ++layer)
        layers.push_back(load_12b_layer(gguf, layer));

    RMSNorm<C12::D> final_norm(
        tensor_loader::load_vector<C12::D>(gguf, "output_norm.weight"), C12::RMS_EPS);
    return Gemma4_12BTransformer(Gemma4_12BTextWeights(
        std::move(token_io), std::move(layers), std::move(final_norm)));
}
