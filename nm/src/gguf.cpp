...
double read_num(Cursor& c, GGUFType t) {
    switch (t) {
        case GGUFType::UINT8:   return c.read<uint8_t>();
        case GGUFType::INT8:    return c.read<int8_t>();
        case GGUFType::UINT16:  return c.read<uint16_t>();
        case GGUFType::INT16:   return c.read<int16_t>();
        case GGUFType::UINT32:  return c.read<uint32_t>();
        case GGUFType::INT32:   return c.read<int32_t>();
        case GGUFType::FLOAT32: return c.read<float>();
        case GGUFType::BOOL:    return c.read<uint8_t>() != 0;
        case GGUFType::UINT64:  return double(c.read<uint64_t>());
        case GGUFType::INT64:   return double(c.read<int64_t>());
        case GGUFType::FLOAT64: return c.read<double>();
        default: throw std::runtime_error("GGUF: non-scalar in read_num");
    }
}
...
GGUFValue read_value(Cursor& c) {
    ...
    if (v.type == GGUFType::STRING) { v.str = c.read_str(); return v; }
...
GGUF::GGUF(const std::string& path) {
    ...
    struct stat st{};
    if (::fstat(fd_, &st) != 0) { ::close(fd_); throw std::runtime_error("GGUF: fstat failed"); }
    size_ = size_t(st.st_size);
    map_ = ::mmap(nullptr, size_, PROT_READ, MAP_PRIVATE, fd_, 0);
    if (map_ == MAP_FAILED) { ::close(fd_); throw std::runtime_error("GGUF: mmap failed"); }

    // keepalive: munmap + close when the last Weight view drops.
    void* mp = map_; size_t sz = size_; int fd = fd_;
    ...
    if (version_ != 3)
        throw std::runtime_error("GGUF: unsupported version " + std::to_string(version_) + " (expect 3)");
    ...
    if ((alignment_ & (alignment_ - 1)) != 0)
        throw std::runtime_error("GGUF: general.alignment is not a power of two");
    ...
    if (header_end > std::numeric_limits<uint64_t>::max() - padding)
        throw std::runtime_error("GGUF: aligned header offset overflows");
    uint64_t data_start = (header_end + padding) & ~padding;
    if (!tensors_.empty() && data_start > size_)
        throw std::runtime_error("GGUF: data section starts past end of file");

    for (auto& t : tensors_) {
        if (t.offset % alignment_ != 0)
            throw std::runtime_error("GGUF: tensor '" + t.name + "' offset is misaligned");
        if (t.offset > uint64_t(size_) - data_start)
            throw std::runtime_error("GGUF: tensor '" + t.name + "' data out of range");
        const uint64_t abs = data_start + t.offset;
        const uint64_t bytes = t.nbytes();
        if (bytes > uint64_t(size_) - abs)
            throw std::runtime_error("GGUF: tensor '" + t.name + "' data out of range");
...
int64_t GGUF::get_int(const std::string& k, int64_t def) const {
    auto it = meta_.find(k); return it == meta_.end() ? def : it->second.as_int();
}
double GGUF::get_float(const std::string& k, double def) const {
    auto it = meta_.find(k); return it == meta_.end() ? def : it->second.as_num();
}
std::string GGUF::get_str(const std::string& k, const std::string& def) const {
    auto it = meta_.find(k); return it == meta_.end() ? def : it->second.str;
}
