...
struct TensorInfo {
    ...
    uint64_t nelem() const {
        ...
        for (uint64_t d : dims) {
            if (d != 0 && n > std::numeric_limits<uint64_t>::max() / d)
                throw std::runtime_error("GGUF: tensor '" + name + "' element count overflows");
            n *= d;
        }
        return n;
    }
    uint64_t nbytes() const {
        ...
        if (n % block != 0)
            throw std::runtime_error("GGUF: tensor '" + name + "' has a partial quant block");
        const uint64_t blocks = n / block;
        const uint64_t bytes_per_block = type_size_bytes(type, block);
        if (blocks != 0 && bytes_per_block > std::numeric_limits<uint64_t>::max() / blocks)
            throw std::runtime_error("GGUF: tensor '" + name + "' byte size overflows");
