...
inline size_t type_size_bytes(GT t, size_t nelem) {
    switch (t) {
        case GT::F32:  return nelem * 4;
        case GT::F16: case GT::BF16: return nelem * 2;
...
inline Scalar dot_block_q4_K(const block_q4_K& b, const Scalar* xb) {
    ...
    int is = 0; const Scalar* x = xb;
#if defined(__ARM_NEON)
    float32x4_t vacc = vdupq_n_f32(0);
    const uint8x16_t mask = vdupq_n_u8(0x0F);
    for (int j = 0; j < (int)QK_K; j += 64) {
        uint8_t sc, m;
        get_scale_min_k4(is + 0, b.scales, sc, m); const float d1 = d*sc, m1 = mn*m;
        get_scale_min_k4(is + 1, b.scales, sc, m); const float d2 = d*sc, m2 = mn*m;
...
inline int8x16_t q6_vals(uint8x16_t nib, uint8x16_t qh16) {
    uint8x16_t hb = Shift == 0 ? vandq_u8(qh16, vdupq_n_u8(3))
                               : vandq_u8(vshrq_n_u8(qh16, Shift ? Shift : 1), vdupq_n_u8(3));
...
inline void dequant_to_f32(GT t, const void* src, Scalar* dst, size_t n) {
    switch (t) {
        case GT::F32: std::memcpy(dst, src, n * 4); return;
        case GT::F16: {
        ...
        case GT::Q4_K: {
            ...
            for (size_t i = 0; i < n / QK_K; ++i) {
                ...
                const uint8_t* q = b[i].qs; int is = 0;
                for (int j = 0; j < (int)QK_K; j += 64) {
                    uint8_t sc, m;
        ...
        case GT::Q6_K: {
            ...
            for (size_t i = 0; i < n / QK_K; ++i) {
                const Scalar d = fp16_to_fp32(b[i].d);
                const uint8_t* ql = b[i].ql; const uint8_t* qh = b[i].qh;
...
inline void quantize_q8_0(const Scalar* x, block_q8_0* b, size_t n) {
    for (size_t i = 0; i < n / QK8_0; ++i) {
        ...
        for (size_t j = 0; j < QK8_0; ++j)
            b[i].qs[j] = int8_t(std::lround(x[i*QK8_0+j] * id));
    }
}
...
public:
    ...
    explicit Weight(MatT<In, Out> f32, std::shared_ptr<const void> keepalive = {})
        : type_(GT::F32), f32_(std::move(f32)), keepalive_(std::move(keepalive)) {
    ...
    Weight(GT type, const void* data, std::shared_ptr<const void> keepalive)
        : type_(type), qp_(static_cast<const uint8_t*>(data)),
          keepalive_(std::move(keepalive)) {
        if (type_ == GT::F32 || type_ == GT::F16 || type_ == GT::BF16)
            throw std::invalid_argument("Weight: expected a quantised type");
    ...
    Vec<In> dequant_row(size_t o) const {
        Vec<In> r;
        if (type_ == GT::F32) { for (size_t i=0;i<In;++i) r[i]=f32_.row(o)[i]; return r; }
