...
inline void quantize_row_q8_K(const float* x, block_q8_K* y, size_t k) {
    ...
    for (size_t i = 0; i < nb; ++i) {
        ...
        for (size_t j = 0; j < QK_K; ++j) y[i].qs[j] = int8_t(std::min(127, nearest_int(iscale * x[j])));
...
inline void quantize_row_q8_0(const float* x, block_q8_0* y, size_t k) {
    ...
    for (size_t i = 0; i < nb; ++i) {
        ...
        for (size_t j = 0; j < QK8_0; ++j) y[i].qs[j] = int8_t(std::roundf(x[i * QK8_0 + j] * id));
    }
}
...
inline float vec_dot_q4_K_q8_K_neon(size_t n, const block_q4_K* x, const block_q8_K* y) {
    ...
    for (size_t i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < (int)QK_K / 64; ++j) {  // 64 elems: 2 scales
            uint8x16_t r0 = vld1q_u8(q4), r1 = vld1q_u8(q4 + 16);
            q4 += 32;
            ...
            a0 = vdotq_s32(a0, lo1, vld1q_s8(q8 + 16));
            q8 += 32;
            int32x4_t a1 = vdotq_s32(vdupq_n_s32(0), hi0, vld1q_s8(q8));
            a1 = vdotq_s32(a1, hi1, vld1q_s8(q8 + 16));
            q8 += 32;
...
inline bool has_i8_kernel(GT t) { return t == GT::Q8_0 || t == GT::Q4_0 || t == GT::Q4_K || t == GT::Q6_K; }
...
Vec<Out> Weight<In, Out>::matvec(VecView<In> x) const {
    ...
    if (!nm_fp32_act() && has_i8_kernel(t)) {  // int8 activations (default)
        ...
        par_for(Out, [&](size_t o) {
            ...
            switch (t) {
                case GT::Q8_0:
                    y[o] = vec_dot_q8_0_q8_0(In, (const block_q8_0*)row, act.q8.data());
                    break;
                case GT::Q4_0:
                    y[o] = vec_dot_q4_0_q8_0(In, (const block_q4_0*)row, act.q8.data());
                    break;
                case GT::Q4_K:
                    y[o] = vec_dot_q4_K_q8_K(In, (const block_q4_K*)row, act.qk.data());
                    break;
                case GT::Q6_K:
                    y[o] = vec_dot_q6_K_q8_K(In, (const block_q6_K*)row, act.qk.data());
                    break;
                default:
                    break;
            }
...
Matrix<Out> Weight<In, Out>::matmul(MatrixView<In> x) const {
    ...
    if (type != GT::Q8_0 && type != GT::Q4_0 && type != GT::Q4_K && type != GT::Q5_K && type != GT::Q6_K) throw std::runtime_error("Weight::matmul: unsupported quantized type");
    if (!nm_fp32_act() && has_i8_kernel(type)) {
        std::vector<ActQ> activations(x.rows());
        par_for(x.rows(), [&](size_t row) { activations[row].from(x.row(row).begin(), In, type); });
        par_for(Out, [&](size_t output) {
            ...
            for (size_t row = 0; row < x.rows(); ++row) {
                ...
                switch (type) {
                    case GT::Q8_0:
                        result = vec_dot_q8_0_q8_0(In, reinterpret_cast<const block_q8_0*>(weight_row), activation.q8.data());
                        break;
                    case GT::Q4_0:
                        result = vec_dot_q4_0_q8_0(In, reinterpret_cast<const block_q4_0*>(weight_row), activation.q8.data());
                        break;
                    case GT::Q4_K:
                        result = vec_dot_q4_K_q8_K(In, reinterpret_cast<const block_q4_K*>(weight_row), activation.qk.data());
                        break;
                    case GT::Q6_K:
                        result = vec_dot_q6_K_q8_K(In, reinterpret_cast<const block_q6_K*>(weight_row), activation.qk.data());
                        break;
                    default:
                        break;
                }
    ...
    par_for(Out, [&](size_t output) {
        ...
        for (size_t row = 0; row < x.rows(); ++row) {
            ...
            switch (type) {
                case GT::Q8_0: {
                    const auto* blocks = reinterpret_cast<const block_q8_0*>(weight_row);
                    for (size_t block = 0; block < In / QK8_0; ++block) result += dot_block_q8_0(blocks[block], &activation[block * QK8_0]);
                    break;
                }
                case GT::Q4_0: {
                    const auto* blocks = reinterpret_cast<const block_q4_0*>(weight_row);
                    for (size_t block = 0; block < In / QK4_0; ++block) result += dot_block_q4_0(blocks[block], &activation[block * QK4_0]);
                    break;
                }
                case GT::Q4_K: {
                    const auto* blocks = reinterpret_cast<const block_q4_K*>(weight_row);
                    for (size_t block = 0; block < In / QK_K; ++block) result += dot_block_q4_K(blocks[block], &activation[block * QK_K]);
                    break;
                }
                case GT::Q5_K: {
                    const auto* blocks = reinterpret_cast<const block_q5_K*>(weight_row);
                    for (size_t block = 0; block < In / QK_K; ++block) result += dot_block_q5_K(blocks[block], &activation[block * QK_K]);
                    break;
                }
                case GT::Q6_K: {
                    const auto* blocks = reinterpret_cast<const block_q6_K*>(weight_row);
                    for (size_t block = 0; block < In / QK_K; ++block) result += dot_block_q6_K(blocks[block], &activation[block * QK_K]);
                    break;
                }
                default:
                    break;
