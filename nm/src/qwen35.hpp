...
constexpr size_t qwen35_param_count() {
    ...
    constexpr size_t recurrent = C::D * (2 * key_width + value_width) +
                                 C::D * value_width + value_width * C::D +
                                 2 * C::D * C::VALUE_HEADS +
                                 C::CONV_WIDTH * (2 * key_width + value_width);
    ...
    return embed + attention_layers * (attention + ff) +
           recurrent_layers * (recurrent + ff);
}

static_assert(qwen35_param_count<Qwen35_4BConfig>() > 3'700'000'000 &&
              qwen35_param_count<Qwen35_4BConfig>() < 4'400'000'000,
              "Qwen 3.5 4B lands on its advertised size");
static_assert(qwen35_param_count<Qwen35_9BConfig>() > 8'400'000'000 &&
              qwen35_param_count<Qwen35_9BConfig>() < 9'800'000'000,
              "Qwen 3.5 9B lands on its advertised size");
...
static_assert(!Qwen35_4BConfig::is_recurrent(Qwen35_4BConfig::L - 1),
              "the final layer must be full attention");
...
public:
    ...
    QwenGatedAttention(Linear<D, GATED_QW> query_gate,
                       PerHeadNorm<Hq, HeadDim, RMSNorm<HeadDim>> query_norm,
                       Linear<D, KW> key,
                       PerHeadNorm<Hkv, HeadDim, RMSNorm<HeadDim>> key_norm,
                       Linear<D, KW> value, Linear<QW, D> output)
        : query_gate_(std::move(query_gate)), query_norm_(std::move(query_norm)),
          key_(std::move(key)), key_norm_(std::move(key_norm)),
          value_(std::move(value)), output_(std::move(output)) {}

    // .first is the normalized query, .second the raw gate.
    HeadPair<Hq, HeadDim> query_and_gate(MatrixView<D> hidden) const {
        HeadPair<Hq, HeadDim> split =
            split_head_pairs<Hq, HeadDim>(query_gate_(hidden).view());
        split.first = query_norm_(split.first.view());
        return split;
    }
    Matrix<KW> key(MatrixView<D> hidden) const {
        return key_norm_(key_(hidden).view());
    }
...
public:
    ...
    QwenGatedDeltaNet(Linear<D, CONV_CHANNELS> qkv, Linear<D, VALUE_WIDTH> gate,
                      Matrix<ConvWidth> conv, Linear<D, Hv> beta,
                      Linear<D, Hv> alpha, Vec<Hv> dt_bias, Vec<Hv> decay_scale,
                      PerHeadNorm<Hv, Dv, RMSNorm<Dv>> output_norm,
                      Linear<VALUE_WIDTH, D> output)
        : qkv_(std::move(qkv)), gate_(std::move(gate)), conv_(std::move(conv)),
          beta_(std::move(beta)), alpha_(std::move(alpha)),
          dt_bias_(std::move(dt_bias)), decay_scale_(std::move(decay_scale)),
          output_norm_(std::move(output_norm)), output_(std::move(output)) {
        if (conv_.rows() != CONV_CHANNELS)
            throw std::invalid_argument("QwenGatedDeltaNet: conv channel mismatch");
        ...
        for (size_t h = 0; h < Hv; ++h)
            if (!(decay_scale_[h] <= 0.f))
                throw std::invalid_argument(
                    "QwenGatedDeltaNet: decay scale must be <= 0 (-exp(A_log))");
    }
    ...
    Matrix<VALUE_WIDTH> normalize_heads(MatrixView<VALUE_WIDTH> mixed) const {
        return output_norm_(mixed);
    }
    ...
    Matrix<Hv> write_strength(MatrixView<D> hidden) const {
        ...
        for (size_t i = 0; i < beta.rows() * Hv; ++i)
            beta.data()[i] = sigmoid(beta.data()[i]);
        return beta;
    }
    Matrix<Hv> decay_gate(MatrixView<D> hidden) const {
        ...
        for (size_t row = 0; row < alpha.rows(); ++row)
            for (size_t h = 0; h < Hv; ++h) {
                ...
                alpha.row_mut(row)[h] =
                    decay_scale_[h] * softplus(raw + dt_bias_[h]);
            }
        return alpha;
    }
...
public:
    ...
    QwenTokenIO(TokenWeight tokens, OutputWeight unembed)
        : tokens_(std::move(tokens)), unembed_(std::move(unembed)) {}

    Vec<C::D> token(TokenId id) const { return tokens_.dequant_row(size_t(id)); }
    Matrix<C::D> tokens(std::span<const TokenId> ids) const {
        return tokens_.gather_rows(ids);
    }
...
using Qwen35Block = TransformerBlock<
    C::D,
    ResidualBranch<C::D, RMSNorm<C::D>, Mixer>,
    ResidualBranch<C::D, RMSNorm<C::D>, GatedMLP<C::D, C::FF>>>;

template <class C>
using QwenAttentionMixer =
    QwenGatedAttention<C::D, C::Hq, C::Hkv, C::HEAD_DIM>;

template <class C>
using QwenRecurrentMixer =
    QwenGatedDeltaNet<C::D, C::KEY_HEAD_DIM, C::VALUE_HEAD_DIM, C::KEY_HEADS,
                      C::VALUE_HEADS, C::CONV_WIDTH>;

template <class C>
using Qwen35Layer = std::variant<Qwen35Block<C, QwenRecurrentMixer<C>>,
                                 Qwen35Block<C, QwenAttentionMixer<C>>>;

template <class C>
struct Qwen35LayerSchedule {
    static void validate(const Qwen35Layer<C>& layer, size_t index) {
        if (layer.index() != (C::is_recurrent(index) ? 0u : 1u))
            throw std::invalid_argument(
                "Qwen35: layer variant disagrees with the hybrid schedule");
    }
};
...
public:
    ...
    void advance(size_t count) {
        if (count > C::CTX - tokens_)
            throw std::length_error("Qwen35State: context exhausted");
        tokens_ += count;
    }

    AttentionEntry& attention(size_t layer) {
        return std::get<AttentionEntry>(entries_[layer]);
    }
    RecurrentEntry& recurrent(size_t layer) {
        return std::get<RecurrentEntry>(entries_[layer]);
    }
    ...
    static constexpr size_t recurrent_floats_per_layer() {
        return DeltaNetState<C::VALUE_HEADS, C::KEY_HEAD_DIM,
                             C::VALUE_HEAD_DIM>::floats;
    }
    static constexpr size_t attention_floats_per_token_per_layer() {
        return 2 * C::Hkv * C::HEAD_DIM;
    }
...
struct Qwen35Architecture {
    ...
    using Weights = TransformerWeights<D, V, L, QwenTokenIO<C>, Qwen35Layer<C>,
                                       RMSNorm<D>, NoArchitectureData,
                                       Qwen35LayerSchedule<C>>;
    ...
    using Rope = RotaryEmbedding<C::HEAD_DIM, C::ROPE_BASE, C::ROTARY_NUM,
                                 C::ROTARY_DEN>;
    ...
    static void forward_layer(const Weights& weights, PrefixState& state,
        ...
        Matrix<D> next = std::visit([&](const auto& layer) {
            return layer.forward(
                ...
                [&](const auto& mixer, MatrixView<D> normalized) {
                    return mix_tokens(mixer, normalized, state, layer_index,
                                      first_position);
                },
                [](const auto& channel, MatrixView<D> normalized) {
    ...
    static Vec<V> output(const Weights& weights,
        ...
        Vec<D> last =
            weights.final_norm()(residual.token(residual.tokens() - 1));
...
private:
    ...
    //     out = A W_o
    static Matrix<D> mix_tokens(const QwenAttentionMixer<C>& attention,
                                MatrixView<D> X, PrefixState& state,
                                size_t layer, size_t first_position) {
        ...
        Matrix<QW> A = attend_and_cache<C::Hq, C::Hkv, Dh>(
            Q.view(), K.view(), V.view(), state.attention(layer),
            first_position, /*sliding_window=*/0,
            /*score_scale=*/1.f / std::sqrt(Scalar(Dh)));

        // The gate is applied to the attended value, before W_o.
        for (size_t i = 0; i < A.rows() * QW; ++i)
            A.data()[i] *= sigmoid(query_gate.second.data()[i]);
    ...
    static Matrix<D> mix_tokens(const QwenRecurrentMixer<C>& mixer,
                                MatrixView<D> X, PrefixState& state,
                                size_t layer, size_t /*first_position*/) {
        ...
        for (size_t t = 0; t < X.rows(); ++t) {
            Vec<Mixer::CONV_CHANNELS> stream =
                entry.conv.step(qkv.row(t), mixer.conv());
            silu(stream);
            ...
            for (size_t hk = 0; hk < Hk; ++hk) {
                query[hk] = l2_normalize<Dk>(
                    slice<Dk>(VecView<Mixer::CONV_CHANNELS>(stream), hk * Dk));
                key[hk] = l2_normalize<Dk>(
                    slice<Dk>(VecView<Mixer::CONV_CHANNELS>(stream),
                              Mixer::KEY_WIDTH + hk * Dk));
            ...
            for (size_t h = 0; h < Hv; ++h) {
                ...
                Vec<Dv> attended = gated_delta_step<Dk, Dv>(
                    entry.delta.head(h), VecView<Dk>(query[hk]),
                    VecView<Dk>(key[hk]),
                    slice<Dv>(VecView<Mixer::CONV_CHANNELS>(stream),
                              2 * Mixer::KEY_WIDTH + h * Dv),
                    gate.row(t)[h], beta.row(t)[h]);
                std::copy(attended.begin(), attended.end(), row + h * Dv);
            }
        }

        // Gated output norm: rmsnorm per head, then the SiLU'd z gate.
        Matrix<Mixer::VALUE_WIDTH> normalized =
            mixer.normalize_heads(mixed.view());
        Matrix<Mixer::VALUE_WIDTH> z = mixer.gate(X);
        silu_in_place(z.mutable_view());
        return mixer.output(
            hadamard(normalized.view(), z.view()).view());
    }
};
