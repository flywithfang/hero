...
QwenAttentionMixer<C> load_attention(const GGUF& gguf, size_t layer) {
    ...
    auto head_norm = [&](const char* suffix) { return PerHeadNorm<RMSNorm<C::HEAD_DIM>>(RMSNorm<C::HEAD_DIM>(tensor_loader::load_vector<C::HEAD_DIM>(gguf, block(layer, suffix)), C::RMS_EPS)); };
    auto kv_head_norm = [&](const char* suffix) { return PerHeadNorm<RMSNorm<C::HEAD_DIM>>(RMSNorm<C::HEAD_DIM>(tensor_loader::load_vector<C::HEAD_DIM>(gguf, block(layer, suffix)), C::RMS_EPS)); };
