...
struct ByteRemap {
    ...
    ByteRemap() {
        ...
        std::array<bool,256> in{}; for (int b : bs) in[b] = true;
        int n = 0;
        for (int b = 0; b < 256; ++b) {
            uint32_t cp;
            if (in[b]) cp = uint32_t(b);
...
const ByteRemap& remap() { static ByteRemap r; return r; }
...
bool in_ranges(uint32_t cp, const uint32_t (*r)[2], size_t n) {
    size_t lo = 0, hi = n;
    while (lo < hi) { size_t m=(lo+hi)/2; if (cp < r[m][0]) hi=m; else if (cp >= r[m][1]) lo=m+1; else return true; }
    return false;
}
...
std::vector<std::string> Tokenizer::pretokenize(const std::string& text) const {
    ...
    std::vector<uint32_t> cp; std::vector<size_t> off;
    ...
    size_t prev = 0;
    auto emit = [&](size_t end){ if (end > prev) spans.push_back({prev,end}); prev = end; };
    ...
    while (pos < N) {
        ...
        if (c == '\'' && pos+1 < N) {
            ...
            if (n1=='s'||n1=='t'||n1=='m'||n1=='d') { pos += 2; emit(pos); continue; }
            if (pos+2 < N) {
        ...
        while (W(pos+nw)) { uint32_t c2 = cp[pos+nw]; if (c2=='\r'||c2=='\n') last_rn = pos+nw+1; nw++; }
        if (last_rn > 0) { pos = last_rn; emit(pos); continue; }
        if (nw > 1 && in(pos+nw)) { pos += nw-1; emit(pos); continue; }
        if (nw > 0) { pos += nw; emit(pos); continue; }
...
std::vector<int32_t> Tokenizer::bpe_encode_chunk(const std::string& piece) const {
    ...
    if (byte_encode_) {
        ...
        for (size_t i = 0; i < cps.size(); ++i)
            sym.push_back(piece.substr(offsets[i], offsets[i + 1] - offsets[i]));
    }
    if (sym.empty()) return {};
    ...
    for (;;) {
        int best_rank = INT32_MAX; size_t best_i = 0; bool found = false;
        for (size_t i = 0; i + 1 < sym.size(); ++i) {
            ...
            if (it != merge_rank_.end() && it->second < best_rank) { best_rank = it->second; best_i = i; found = true; }
        }
        if (!found) break;
    ...
    std::vector<int32_t> ids; ids.reserve(sym.size());
    for (auto& s : sym) {
        int32_t id = id_of(s);
        if (id >= 0) ids.push_back(id);
        else for (unsigned char ch : s) {
            std::string fallback;
            if (byte_encode_) {
...
std::string Tokenizer::decode1(int32_t id) const {
    ...
    std::vector<uint32_t> cp; std::vector<size_t> off; utf8_decode(t, cp, off);
    std::string out;
    for (uint32_t c : cp) {
        auto it = remap().cp_to_byte.find(c);
        if (it != remap().cp_to_byte.end()) out += char(it->second);
        else utf8_append(out, c);
    }
    return out;
}
