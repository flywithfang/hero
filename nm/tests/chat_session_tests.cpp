...
int main(int argc, char** argv) {
    ...
    {
        const GemmaChatTurn turn = render_gemma_chat_turn(
            "Hello", true, "Be concise.", false);
        check(turn.before_image ==
              "<|turn>system\n<|think|>\nBe concise.<turn|>\n<|turn>user\n",
    ...
    {
        const GemmaChatTurn turn = render_gemma_chat_turn(
            "Describe it.", false, "ignored", true);
        check(turn.before_image == "<|turn>user\n<|image>",
              "later image turn opens an image segment without repeating system");
    ...
    std::printf("== application image boundary ==\n");
    {
        ...
        check(image.width() == 2 && image.height() == 2,
              "dependency-free PPM loader preserves dimensions");
        check(image.pixels()[0] == 255 && image.pixels()[1] == 0 &&
              image.pixels()[2] == 0 && image.pixels()[9] == 255,
              "PPM loader produces interleaved RGB for the encoder contract");
        if (argc == 3) {
            const RGBImage platform_image = load_rgb_image(argv[2]);
            check(platform_image.width() == 2 && platform_image.height() == 2,
                  "platform decoder accepts a common image format");
            const bool same_pixels = std::equal(
                platform_image.pixels().begin(), platform_image.pixels().end(),
    ...
    std::printf("== Gemma channel streaming ==\n");
    {
        ...
        formatter.flush();
        check(rendered == "\n[thought]\nHere is the reasoning.\n[final]\nThe answer.",
              "channel protocol becomes readable streaming sections");
    }

    std::printf("\n%s (%d failures)\n",
                failures ? "CHAT SESSION TESTS FAILED" : "ALL CHAT SESSION TESTS PASSED",
                failures);
    return failures ? 1 : 0;
}
