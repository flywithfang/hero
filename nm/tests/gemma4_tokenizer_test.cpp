...
static void check_case(const Tokenizer& tokenizer, const char* text,
                       std::initializer_list<int32_t> expected,
                       bool expect_roundtrip = true) {
    ...
    if (!ids_match) {
        ++failures;
        std::printf("    got:"); for (int32_t id : actual) std::printf(" %d", id);
        std::printf("\n    want:"); for (int32_t id : want) std::printf(" %d", id);
        std::printf("\n");
        return;
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        check_case(tokenizer, "caf\xC3\xA9 \xE4\xB8\xAD\xE6\x96\x87 \xF0\x9F\x91\x8B",
                   {2, 123125, 236859, 17346, 237364, 155818});
        check_case(tokenizer, "  spaced   text", {2, 138, 169862, 139, 1005});
        check_case(tokenizer, "x += 42; // code",
                   {2, 236781, 3323, 236743, 236812, 236778, 236793, 973, 3393});
        ...
        std::printf("%s (%d failures)\n",
                    failures ? "GEMMA4 TOKENIZER TESTS FAILED" : "ALL GEMMA4 TOKENIZER TESTS PASSED",
                    failures);
        return failures ? 1 : 0;
