...
int main() {
    ...
    check(
        patches.rows() == 1 &&
            patches.row(0)[0] == -1.f &&
            patches.row(0)[4] == 2.f * (1.f / 255.f) - 1.f &&
            patches.row(0)[8] == 2.f * (2.f / 255.f) - 1.f,
        "patchify produces channel-major patch rows");
    ...
    check(
        pooled.rows() == 1 &&
            std::fabs(pooled.row(0)[0] - 2.5f) < 1e-6f,
        "2-D average pooling preserves the grid interpretation");
    ...
    Matrix<2> attended =
        scaled_dot_product_attention<1, 2, 2>(
            queries.view(), keys.view(), values.view(), 1.f);
    const Scalar high = std::exp(1.f) / (std::exp(1.f) + 1.f);
    check(
        std::fabs(
            attended.row(0)[0] -
            (high * 10.f + (1.f - high) * 30.f)) < 1e-5f &&
        std::fabs(
            attended.row(1)[1] -
            ((1.f - high) * 20.f + high * 40.f)) < 1e-5f,
        "full attention is softmax(QK^T)V");
