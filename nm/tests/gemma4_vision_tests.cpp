...
int main() {
    ...
    const PositionTable2D<2, 3> table(raw.data(), std::make_shared<int>(1));
    ...
    const Matrix<12> patches = patchify_rgb<2>(pixels, 2, 2);
    ...
    const Matrix<1> grid{{1.f}, {2.f}, {3.f}, {4.f}};
    const Matrix<1> pooled = average_pool_2d<2>(grid.view(), 2, 2);
    ...
    const Matrix<2> queries{{1.f, 0.f}, {0.f, 1.f}};
    const Matrix<2> keys{{1.f, 0.f}, {0.f, 1.f}};
    const Matrix<2> values{{10.f, 20.f}, {30.f, 40.f}};
    const Matrix<2> attended = scaled_dot_product_attention<1, 2, 2>(queries.view(), keys.view(), values.view(), 1.f);
