...
static float frand(float lo, float hi) {
    std::uniform_real_distribution<float> U(lo, hi); return U(rng);
}
...
int main() {
    std::printf("== BF16 conversion ==\n");
    {
        ...
        for (size_t i = 0; i < x.size(); ++i)
            exact = exact && y[i] == bf16_to_fp32(b[i]);
        check(exact && type_size_bytes(GT::BF16, x.size()) == 2 * x.size(),
              "BF16 widens exactly and occupies two bytes");
    }
    ...
    std::printf("== Q8_0 round-trip ==\n");
    {
        ...
        for (size_t i = 0; i < N; ++i) { maxerr = std::max(maxerr, std::fabs(x[i]-y[i])); maxabs = std::max(maxabs, std::fabs(x[i])); }
    ...
    std::printf("== kernel consistency: dot_block == dot(dequant, x) ==\n");
    {
        ...
        // Q8_0
        { block_q8_0 blk; auto w = rand_block_and_dequant(GT::Q8_0, blk);
          std::vector<Scalar> x(32); for (auto& v:x) v=frand(-2,2);
          Scalar k = dot_block_q8_0(blk, x.data());
          Scalar r = 0; for (int i=0;i<32;++i) r += w[i]*x[i];
          check(rel(k,r) < 1e-5f, "Q8_0 kernel == dequant-dot"); }
        // Q4_0
        { block_q4_0 blk; auto w = rand_block_and_dequant(GT::Q4_0, blk);
          std::vector<Scalar> x(32); for (auto& v:x) v=frand(-2,2);
          Scalar k = dot_block_q4_0(blk, x.data());
          Scalar r = 0; for (int i=0;i<32;++i) r += w[i]*x[i];
          check(rel(k,r) < 1e-5f, "Q4_0 kernel == dequant-dot"); }
        // Q4_K
        { block_q4_K blk; auto w = rand_block_and_dequant(GT::Q4_K, blk);
    ...
    std::printf("== int8 activation path: NEON == scalar (ggml-generic clone) ==\n");
    {
        ...
        std::vector<block_q8_K> xk(n / QK_K); quantize_row_q8_K(x.data(), xk.data(), n);
        std::vector<block_q8_0> x8(n / QK8_0); quantize_row_q8_0(x.data(), x8.data(), n);
        std::vector<block_q8_0> w8(n / QK8_0); quantize_q8_0(w.data(), w8.data(), n);
        std::vector<block_q4_0> w4(n / QK4_0); quantize_q4_0(w.data(), w4.data(), n);
        ...
        for (auto& b : w4k) { auto* p=(uint8_t*)&b; for (size_t i=0;i<sizeof b;++i) p[i]=brand();
                              b.d = fp32_to_fp16(0.01f); b.dmin = fp32_to_fp16(0.01f); }
        for (auto& b : w6k) { auto* p=(uint8_t*)&b; for (size_t i=0;i<sizeof b;++i) p[i]=brand();
                              b.d = fp32_to_fp16(0.01f); }
        auto rel = [](float a, float b){ return std::fabs(a-b)/(std::fabs(a)+std::fabs(b)+1e-9f); };
#if defined(__ARM_FEATURE_DOTPROD)
        check(rel(vec_dot_q4_K_q8_K_neon(n, w4k.data(), xk.data()),
                  vec_dot_q4_K_q8_K_scalar(n, w4k.data(), xk.data())) < 1e-5f, "q4_K neon == scalar");
        check(rel(vec_dot_q6_K_q8_K_neon(n, w6k.data(), xk.data()),
                  vec_dot_q6_K_q8_K_scalar(n, w6k.data(), xk.data())) < 1e-5f, "q6_K neon == scalar");
#endif
        // int8 dot vs fp32 dot on the same dequantised weights: bounded by
        // activation quantisation error (~1%), sanity not exactness.
        std::vector<Scalar> wd(n); dequant_to_f32(GT::Q8_0, w8.data(), wd.data(), n);
        float f32dot = 0; for (size_t i=0;i<n;++i) f32dot += wd[i]*x[i];
        check(rel(vec_dot_q8_0_q8_0(n, w8.data(), x8.data()), f32dot) < 2e-2f, "q8_0 int8 ~ fp32 dot");
        dequant_to_f32(GT::Q4_0, w4.data(), wd.data(), n);
        f32dot = 0; for (size_t i=0;i<n;++i) f32dot += wd[i]*x[i];
    ...
    {
        float mx = 0;
        for (float v : {0.0f, 1.0f, -1.0f, 0.5f, 3.14159f, 65504.0f, 1e-4f, -2.5f})
            mx = std::max(mx, std::fabs(v - fp16_to_fp32(fp32_to_fp16(v))) / (std::fabs(v)+1e-6f));
