...
static int run(Transformer<Architecture> transformer, const Tokenizer& tokenizer,
               const std::vector<int32_t>& ids, size_t generate_count) {
    ...
    std::printf("\nnext_token: %zu\npiece: %s\nprefill: %.3fs (%zu tokens, %.1f tok/s)\n",
                ...
                ids.size(), double(ids.size()) / elapsed);
    ...
    if (generate_count > 1) {
        ...
        std::printf("\ntext: %s\ndecode: %.3fs (%zu tokens, %.2f tok/s)\n",
                    text.c_str(), decode_seconds, produced,
                    double(produced) / decode_seconds);
    }
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (blocks == Gemma4_12BTextConfig::L) {
            ...
            return run<Gemma4_12BTextConfig, Gemma4_12BArchitecture>(
                gemma4_loader::load_12b_text(gguf), tokenizer, ids, generate_count);
        }
        if (blocks == Gemma4E4BTextConfig::L) {
            std::fprintf(stderr, "Gemma 4 E4B (%zu layers)\n", blocks);
            return run<Gemma4E4BTextConfig, Gemma4E4BArchitecture>(
                gemma4_loader::load_e4b_text(gguf), tokenizer, ids, generate_count);
        }
