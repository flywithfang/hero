    ...
    if (v.type == GGUFType::ARRAY) {
        ...
        for (size_t i = 0; i < show; ++i) {
            ...
            if (v.elem_type == GGUFType::STRING) {
                ...
                std::printf("%s", fmt_num(v.elem_type, v.arr_num[i]).c_str());
        }
...
int main(int argc, char** argv) {
    ...
    if (mode == "--values") {
        ...
        try {
            ...
            std::printf("%s  %s  nelem=%zu\n", argv[3], gt_name(t.type), size_t(t.nelem()));
            for (size_t i = 0; i < n; ++i) std::printf("  [%zu] %.9g\n", i, double(values[i]));
            return 0;
    ...
    try {
        ...
        if (mode != "--meta") {
            ...
            for (const auto& t : g.tensors()) {
                std::string shape;
                for (size_t i = 0; i < t.dims.size(); ++i) shape += (i ? "x" : "") + std::to_string(t.dims[i]);
                std::printf("  %-32s %-18s %-6s %14llu %14llu\n", t.name.c_str(), shape.c_str(), gt_name(t.type), (unsigned long long)t.nbytes(), (unsigned long long)t.offset);
                total += t.nbytes();
            }
